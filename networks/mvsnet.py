...
class FPNFeature(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        self.conv1 = nn.Sequential(
            ...
            MambaConv2d(in_channels=base_channels * 2),
            MambaConv2d(in_channels=base_channels * 2)
        )

        self.conv2 = nn.Sequential(
            Conv2d(base_channels * 2, base_channels * 4, 5, stride=2, padding=2),
            MambaConv2d(in_channels=base_channels * 4),
            MambaConv2d(in_channels=base_channels * 4)
        )
...
class UNet3DCNNReg(nn.Module):
    def __init__(self, args, in_channels=8, **kwargs):
        ...
        self.conv2 = InceptionConv3d(in_channels=base_channels * 2)
        self.conv4 = InceptionConv3d(in_channels=base_channels * 4)
        self.conv6 = InceptionConv3d(in_channels=base_channels * 8)
        # 下采样
...
class UniformSampler(nn.Module):
    ...
    def forward(self, last_outs, shape, interval_base=None, **kwargs):
        ...
        depth_hypotheses = last_depth_min.unsqueeze(1) + (torch.arange(0, self.num_hypotheses, device=last_depth.device,
                                                                       dtype=last_depth.dtype, requires_grad=False).reshape(1, -1, 1, 1) * new_interval.unsqueeze(1))

        # depth_hypotheses = F.interpolate(depth_hypotheses, shape, mode='bilinear', align_corners=False)
        depth_hypotheses = F.interpolate(depth_hypotheses.unsqueeze(1), [self.num_hypotheses, shape[0], shape[1]], mode='trilinear', align_corners=Align_Corners_Range).squeeze(1)
        return depth_hypotheses
...
#                 "depth_hypotheses": depth_hypotheses, "depth_mode": "regression", "depth_values": depth_hypotheses,
...
#                 # if self.args.refine:
#                 #     epoch = random.randint(0, 20)  # 1/2的噪声小于0.1
...
#                 else:
#                     ref_img, filter_mask = random_patch_mask(ref_img, patch_size=[16, 32, 64], mask_ratio=1 / 3)
#             else:
#                 ref_img, filter_mask = random_image_mask(ref_img, filter_size=(ref_img.size(2) // 3, ref_img.size(3) // 3))
#             imgs_icc[:, 0] = ref_img
#             imgs = imgs_icc
