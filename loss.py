...
class ICCLossMultiStage(nn.Module):
    ...
    def forward(self, data, outputs, epoch_idx, **kwargs):
        if "output2" not in outputs:
            return torch.tensor(0.0, dtype=torch.float32, device=data["imgs"].device, requires_grad=False), {}
        ...
        if self.args.refine:
            w_icc = 0.32  # 仅仅用于refine的时候
        else:
            # w_icc = 0.32
            w_icc = adjust_w_icc(epoch_idx, self.args.w_icc, self.args.max_w_icc)
        # w_icc = adjust_w_icc1(epoch_idx, self.args.w_icc, self.args.max_w_icc)
