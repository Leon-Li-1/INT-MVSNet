...
parser.add_argument('--testpath', default="/home/lida/lida/2Ddataset/dtu_data/DTU/dtu_test", help='testing data dir for some scenes')
...
parser.add_argument('--logdir', default='./outputs/dtu/jiayou_test', help='the directory to save checkpoints/logs')
parser.add_argument('--nolog', action='store_true', help='do not logging into .log file')
parser.add_argument('--plydir', default='./outputs/dtu/jiayou_test/pcd_fusion_plys/', help='output dir')

parser.add_argument('--num_worker', type=int, default=4, help='depth_filer worker')

parser.add_argument('--conf', type=float, nargs='+', default=[0.1, 0.15, 0.8], help='prob confidence, for pcd and dypcd')
...
parser.add_argument('--levels', type=int, default=3, help='num of stages')
...
def check_geometric_consistency(args, depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src, src_confidence):
    ...
    for i in range(2, 11):
        ...
        # mask = np.logical_and(np.logical_and(dist < args.img_dist_thres + (i - 2) * 0.05, relative_depth_diff < 0.0005 + (i - 2) * 0.0001), sample_confi_src > 0.15)
...
def filter_depth(pair_folder, scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        ref_img = read_img(os.path.join(scan_folder, 'images/{:0>8}.png'.format(ref_view)))
...
def pcd_filter_worker(scan):
    ...
    #     'scan10': [0.1, 0.15, 0.6],
    #     'scan11': [0.1, 0.15, 0.5],
    #     'scan12': [0.1, 0.15, 0.55],
    #     'scan13': [0.1, 0.15, 0.65],
    #     'scan15': [0.1, 0.15, 0.6],
    #     'scan23': [0.1, 0.15, 0.65],
    #     'scan24': [0.1, 0.15, 0.6],
    #     'scan29': [0.1, 0.15, 0.5],
    #     'scan32': [0.1, 0.15, 0.5],
    #     'scan33': [0.1, 0.15, 0.5],
    ...
    #     'scan75': [0.1, 0.15, 0.5],
    #     'scan77': [0.1, 0.15, 0.5],
    #     'scan110': [0.1, 0.15, 0.7],
    #     'scan114': [0.1, 0.15, 0.75],
    #     'scan118': [0.1, 0.15, 0.75],
    # }
    conf = {
        'scan1': [0.5, 0.5, 0.8],
        'scan4': [0.5, 0.5, 0.6],
