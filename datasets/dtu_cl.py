...
class MVSDataset(Dataset):
    ...
    def define_transforms(self):
        self.transform_aug = v2.Compose([
            v2.ColorJitter(brightness=1, contrast=1, saturation=0.5, hue=0.5),
            # v2.ColorJitter(brightness=0.8, contrast=0.8, saturation=0.35, hue=0.3),
            v2.ToTensor(),
            RandomGamma(min_gamma=0.5, max_gamma=2.0, clip_image=True),
            # PolarizationSim(max_angle=180, strength_range=(0.5, 0.8)),
            # SmartGamma(min_gamma=0.5, max_gamma=2.0, clip_thresh=0.01),
