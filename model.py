...
from filter import pcd_filter  # NOQA
...
class Model:
    def __init__(self, args):
        ...
        self.network = MVSNet(args).to(self.device)
        ...
        if self.args.resume is True and self.args.start_ckpts is None and args.test is False:
            saved_models = [fn for fn in os.listdir(args.log_dir) if fn.endswith(".ckpt")]
            saved_models = sorted(saved_models, key=lambda x: int(x.split('_')[-1].split('.')[0]))
            loadckpt = os.path.join(args.log_dir, saved_models[-1])
            print("resuming: " + loadckpt)
            checkpoint = torch.load(loadckpt, map_location=torch.device("cpu"), weights_only=False)
            if not (self.args.val or self.args.test):
        ...
        if args.resume is False and args.start_ckpts is not None and args.test is False:
            if not os.path.exists(args.start_ckpts):
                raise NotImplementedError('no checkpoint file from path %s...' % args.start_ckpts)
            print(f'Loading weights from {args.start_ckpts}...')
            # load state dict
            state_dict = torch.load(args.start_ckpts, map_location='cpu', weights_only=False)
            ...
            self.network_without_ddp.load_state_dict(base_ckpt, strict=False)
        if args.resume is True and args.start_ckpts is not None:
            raise ValueError(
                '--resume and --start_ckpts cannot be both activate')

        if args.test and args.loadckpt is not None:
            print(f'Loading weights from {args.loadckpt}...')
            state_dict = torch.load(args.loadckpt, map_location=torch.device("cpu"), weights_only=False)
            self.network_without_ddp.load_state_dict(state_dict['model'])
        if args.test is False and args.loadckpt is None:
            raise ValueError(
                '--for test the loadckpt must be not None')
    ...
    def train_epoch(self, epoch):
        ...
        if is_main_process():
            pwidgets = [progressbar.Percentage(), " ", progressbar.Counter(format='%(value)02d/%(max_value)d'), " ", progressbar.Bar(), " ",
                        ...
                        progressbar.Variable('Loss', width=1), ",", progressbar.Variable('Abs', width=1), ",", progressbar.Variable('Th2', width=1), ",",
        ...
        for batch, data in enumerate(self.train_loader):
            ...
            if is_main_process():
                ...
                pbar.update(batch, LR=self.optimizer.param_groups[0]['lr'],
                            ...
                            Abs="{:.3f}|{:.3f}".format(scalar_outputs["abs_depth_error"], avg_scalars.avg_data["abs_depth_error"]),
    ...
    def validate(self, epoch=0):
        ...
        if is_main_process():
            pwidgets = [progressbar.Percentage(), " ", progressbar.Counter(format='%(value)02d/%(max_value)d'), " ", progressbar.Bar(), " ",
                        ...
                        progressbar.Variable('Abs', width=1), ",", progressbar.Variable('Th2', width=1), ",",
                        progressbar.Variable('Th4', width=1), ",", progressbar.Variable('Th8', width=1)]
